...
対話はセッション(主題)単位で管理さる。最終的な開発段階において、各セッションはBotによってスレッドとして作成・紐づけられる。
...
|セッション終了・削除|/delete_session||指定したセッション(タグ)を削除。対応スレッドも閉鎖または削除|
...
|文脈保存|自動|----|各セッションごとにユーザーの発話を蓄積し、文脈を継続学習・反映|
|文脈応答|自動|----|ユーザーの発話に応じ、該当セッションの文脈をもとに応答を生成|
